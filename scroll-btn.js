(function () {
  function css() {
    ...
    s.textContent =
      ...
      "position:absolute;left:.2rem;top:.15rem;right:auto;width:3.2rem;height:3.2rem;" +
      "font-size:1.55rem;line-height:1;z-index:6;background:transparent;border:0;padding:0;" +
      "display:inline-flex;align-items:center;justify-content:center}" +
      "#candleBtn{position:absolute;right:.2rem;top:.15rem;left:auto;width:3.2rem;height:3.2rem;z-index:6}";
  ...
  function place() {
    ...
    if (scroll) {
      scroll.textContent = "\uD83D\uDCDC";
      scroll.style.left = ".2rem";
      scroll.style.right = "auto";
      if (head && scroll.parentNode !== head) head.appendChild(scroll);
    }
    if (candle && head && candle.parentNode !== head) head.appendChild(candle);
  }
  function boot() {
    place();
