(function () {
  const WINDOWS = [
    { id: "morning", start: 5, end: 9, label: "05" },
    { id: "midday", start: 12, end: 15, label: "12" },
    { id: "night", start: 21, end: 24, label: "21" }
  ];
    ...
    if (demoMin != null) {
      ...
      for (let i = 0; i < WINDOWS.length; i++) {
        ...
        if (h >= w.start && h < w.end) {
          period = w.id === "midday" ? "midday" : w.id;
          closesAt = String(w.end).padStart(2, "0") + ":00";
        }
      }
      return {
        date: date,
        ...
        prevPeriod: h >= 9 && h < 12 ? "morning" : h >= 15 && h < 21 ? "midday" : h < 5 ? "night" : null,
        prevEnd: h >= 9 && h < 12 ? "09:00" : h >= 15 && h < 21 ? "15:00" : h < 5 ? "24:00" : null
      };
    }
  ...
  function inject() {
    ...
    s.id = "day-rail-css";
    s.textContent =
      ".day-rail{margin:.15rem 1.1rem .55rem;user-select:none}" +
      ...
      ".day-rail-track{position:relative;height:14px;background:rgba(42,33,24,.12);border:1px solid var(--rule);overflow:hidden}" +
      "html[data-theme='dark'] .day-rail-track{background:rgba(243,234,214,.08)}" +
      ".day-rail-win{position:absolute;top:0;bottom:0;background:var(--gold,#8a6d3b);opacity:.55}" +
      ".day-rail-win.is-open{opacity:.92;background:var(--rubric,#7a2418)}" +
      ".day-rail-fill{position:absolute;top:0;bottom:0;left:0;background:var(--ink,#2a2118);opacity:.28;pointer-events:none}" +
      ".day-rail-needle{position:absolute;top:-3px;bottom:-3px;width:2px;background:var(--rubric,#7a2418);transform:translateX(-1px);z-index:2}" +
      ".day-rail-ticks{position:relative;height:1.05rem;margin-top:.12rem;font-size:.58rem;letter-spacing:.08em;color:var(--ink-soft)}" +
  ...
  function paintRail() {
    ...
    document.querySelectorAll(".day-rail-win").forEach(function (el) {
      ...
      const w = WINDOWS.filter(function (x) {
        return x.id === id;
      })[0];
      const h = hoursNow();
  ...
  function mountRail() {
    ...
    box.innerHTML =
      ...
      '<span class="day-rail-needle" id="dayRailNeedle"></span>' +
      "</div>" +
      '<div class="day-rail-ticks">' +
      '<span style="left:0">00</span>' +
      '<span style="left:20.833%">05</span>' +
    ...
    track.addEventListener("click", function (e) {
      ...
      setDemoMin(Math.round(x * 1440));
    });
    document.getElementById("railStep").addEventListener("click", function () {
      stepDemo();
    });
    document.getElementById("railWalk").addEventListener("click", function () {
      toggleWalk();
    });
    paintRail();
  }
  ...
  function showFilm(kind) {
    ...
    stage.innerHTML =
      "<h3>" +
      (kind === "morning" ? "Morning film" : "Noon film") +
      "</h3>" +
      '<p class="film-title">' +
      pack.title +
      "</p>" +
      ...
      pack.id +
      '?rel=0&modestbranding=1&playsinline=1" allow="accelerometer; autoplay; encrypted-media; picture-in-picture" allowfullscreen></iframe></div>';
  }
  ...
  function applyOffice() {
    ...
    if (n.period === "morning") {
      ...
      if (closed) {
        ...
        if (body)
          body.textContent =
            "Next office " + (n.nextAt || "") + " · " + (n.nextPeriod || "");
      }
      showFilm(null);
    }
  ...
  function setDemoMin(min) {
    demoMin = Math.max(0, Math.min(1439, min));
    applyOffice();
  }

  function stepDemo() {
    stopAt = (stopAt + 1) % STOPS.length;
    setDemoMin(STOPS[stopAt]);
  }
